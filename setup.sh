...
# Primero voy a motrar cual era y despues lo cambio
    
    echo "El nombre del Hostname actual es: $(hostname)"
    ...
    echo "El nombre del Hostname ahora es: $(hostname)"
...
# Valido si existe el usuario llamado asi, si existe le doy los permisos, sino lo creo con los permisos
    ...
    if  grep -q "webexperto:" /etc/passwd; then
        ...
        echo -e "\nwebexperto, contraseña: superusuario1" | chpasswd
...
#--------------------------------------------------------------------------------------------------------------------------------------
# Instalamos CM
# Validamos que cm este instalado, en caso contrario, lo instalamos

    if ! command -v mc &> /dev/null; then
        
        echo -e "CM no esta instalado\n"
        echo -e "Vamos a instalarlo\n"

        sudo apt install -y mc

        echo -e "El CM ya se instalo correctamente"

    else

        # En caso de estar instalado, mostramos un mensaje que lo indique 

        echo -e "El CM esta instalado :D"
